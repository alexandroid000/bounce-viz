...
patrolling, coverage, and localization. In addition to satisfiability, this
approach also calculates the maximum allowable uncertainty in the robot's
actuation, providing design constraints for different environments and tasks.
\end{abstract}
...
mobile robots. In the first row, $f(v_i, \theta) = \theta$. In the second row,
...
perform the rotation again until its heading points into the free space.
}
\end{figure}
...
\begin{definition}
A \textbf{bounce law} is a function which returns the outgoing heading angle of
the robot. It is a function of the robot's incoming heading (the angle $\alpha$)
and a control parameter $\theta$. For simplicity, we specify bounce laws in the
frame where the environment normal is aligned with the positive $y$ axis and the
robot's point of contact with the wall is the origin. See Figure \ref{fig:bex} for
some example bounce laws.

We may also extend this definition to a \textbf{nondeterministic bounce law}, which
returns a set of outgoing heading angles, any one of which may be executed by
the robot.
\end{definition}
...
robot's possible sensor readings.
\end{definition}
...
\begin{proposition} big O complexity of graph construction and queries
\end{proposition}

\begin{lemma}
graph contains at least one cycle (under all constraints on angle range?)
\end{lemma}
