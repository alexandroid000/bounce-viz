...
\For{$v_r$ in $reflex\_verts$}
    \State $v_{new} \gets$ \Call{ShootRay}{$v_{r-1}, v_r$} \Comment{edges
adjacent to reflex vertex induce transitions}
    \State $v_{new} \gets$ \Call{ShootRay}{$v_{r+1}, v_r$}
...
Specifically, the existence of an edge $e_{ij} \in E(G)$ implies that from any
...
This angle range can be easily constructed through simple trigonometry for a
given point and target segment.
\end{proof}

\begin{lemma}
Given two mutually visible segments $i,j \in \partial P$, there exists a range 
of angles from which a transition from $i$ to $j$ is possible from an open interval 
of $i$.
...
to a family of strategies which take a robot from $x$ to $y$.
\end{lemma}

\begin{proof}
Each edge in $G$ corresponds to a family of bounce laws. 

If the robot is known to be in an interval $x \in \partial P$, which lies
entirely within one of the partitions induced by Algorithm \ref{algo:insert}.


When a navigation strategy $\pi$ is executed from $x$, it describes a sequence
of (possibly nondeterministic) bounce laws, $b_0, \ldots, b_k$. Each of these
bounce laws causes a point or interval along $\partial P$ to be mapped to
another point or interval. 
\end{proof}
...
\begin{lemma}
localization is finding a nondeterministic strategy that produces paths all
...
\subsubsection{Relative Angle Bouncing}
