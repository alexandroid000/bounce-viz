\documentclass[]{svproc}  % Comment this line out
...
\authorrunning{Alexandra Nilles et al.} % abbreviated author list (for running head)
...
any particular area more frequently than others. A robot which is collecting
data to monitor a space (such as humidity, wireless signal, radioactivity levels) may be
...
iRobot vacuum models. The first approach is very powerful, but also
resource-intensive and difficult to reason about formally; the second approach 
is more efficient but it is not as immediately clear how to accomplish tasks
such as navigation and loop-closure. We imagine a dual
...
an algorithm such as SLAM. Second, the global geometry is processed to produce
...
for vacuuming, pool cleaning, lawn mowing, and similar applications. It is a
form of \emph{compliant motion}.
...
boundary, at which point it is able to rotate in place, with some bounded nonzero nondeterministic
error. They then generate a strategy as a sequence of bounce angles which can 
navigate the robot between given start and end locations in a known environment. 
They introduce the idea of "safe actions," which place
...
the diagram without crossing regions within segments ($0 < \theta < 0.6$ and $2.5 <
\theta < \pi$). If we can draw such lines across the region of a single
...
draw such line across the entire diagram, this indicates that this property
holds true for the entire polygon. We call such transitions \emph{safe
bounces}, since for a range of $\theta$, $f(s,\theta)$ maps from one equivalence
class on the boundary to another.
...
\subsection{Path Families}

One major advantage of this approach is that it allows us to characterize
properties of families of paths the robot may take.

For example, the following two paths are produced by different bounce
strategies, but 
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{figures/bouncerange_min.pdf}
...
\begin{definition}
Two line segements $i$ and $j$ are \textbf{entirely visible} to each other iff
...
\State $path \gets$ \Call{PreImageBackchain}{$BVD$, $G$, $S$}
\State $strategy \gets$ \Call{getStrategy}{$path$, $k$}
\Comment $k$ is an upper bound on the path length
%\State \Return \Call{argmax}{strategies, \textsc{}}
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
Example: using cycles to control uncertainty
...
{\color{red} ergodicity / chaotic trajectories}
