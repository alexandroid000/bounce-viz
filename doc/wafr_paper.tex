...
approach, where global geometrical information is either provided \emph{a
priori} to the system (for instance, from a floor plan), or calculated online by
an algorithm such as SLAM, but then the global geometry is processed to produce
a strategy which uses minimal processing power and only robust local
sensors.
...
for vacuuming, pool cleaning, lawn mowing, and similar applications. In robotics, 

{\color{red} mason, erdmann, goldberg, funnelling and squeeze grasping.
lozano-perez, mason, taylor compliant motion.}
...
equivalence classes induced by $\thicksim$.
