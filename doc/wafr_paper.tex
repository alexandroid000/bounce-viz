...
\phi_i)}{\sin(\pi-\theta_i)}$, and the constraints on the angle range
...
do BFS this way until goal set is found - could be multiple paths with same num
ber of bounces that get to goal set

Then, trim backwards from goal set (how?)

But how do we find the subinterval of $i$ for which such a transition is
guaranteed?
