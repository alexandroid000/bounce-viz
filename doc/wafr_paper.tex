...
the segments is a contraction mapping if and only if $\phi/2 < \theta < \pi$ or $0 < \theta
< \phi/2$.
\end{lemma}
...
\State $G\_nav \gets$ \Call{PartitionPoly}{$poly$}
\State $bounce\_seq \gets$ \Call{BFS}{$G\_nav$, $criterion$}
\For{$v_r$ in $reflex\_verts$}
\EndFor
\EndProcedure
\end{algorithmic}
