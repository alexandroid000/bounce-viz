...
reflections (as if the walls of the polygon are mirrors) \cite{Aronov1996,prasad1998visibility}.
...
polygons with bounce visibility graph edge-set size of $O(n^4)$ as follows.
...
\begin{definition}
Let $\phi_{ij}$ be the interior angle between two segments $i$ and $j \in \partial P$ if they are extended 
...
position on $e_k$ after bouncing $k$ consecutive edges as $b^{k}(x)$ and
$b^{k}(y)$ respectively. $\triangle b^{k}(x)v_{k+1}b^{k+1}(x)$ and
$\triangle b^{k}(y)v_{k+1}b^{k+1}(y)$ are similar triangles since they share a
...
\begin{eqnarray*}
\frac{d(b^{n}(x)), b^{n}(y))}{d(b^{0}(x), b^{0}(y))} = \prod_{i = 0}^{n-1}
C(\theta, \phi_{i}).
\end{eqnarray*}

If $\prod_{i = 0}^{n-1} C(\theta, \phi_{i}) < 1$, then $b^k(x)$ is a contraction
mapping from $e_0$ back to iteself, and by the Banach fixed-point theorem, it
has a unique fixed point \cite{Granas2003}.

If $|\frac{\sin(\theta)}{\sin(\Phi_{i}-\theta)}|<1$, which is equivalent to
$\theta < \frac{\Phi_{i}}{2}$, for $i = 0, 1, \dots, n-1$, then
$|\Pi_{i = 0}^{n-1}\frac{\sin(\theta)}{\sin(\Phi_{i}-\theta)}|<1$ and we have a
contraction mapping.

Therefore, \begin{equation*}
\forall \theta \in (0, \min(\min_{i = 0, 1, \dots, n-1}(\angle v_{i+2}v_{i}v_{i+1}),
\frac{\phi_{P,min}}{2})),\end{equation*} the fixed-angle bouncing strategy with $\theta$ leads to a convex $n$-cycle regardless of the robot's start position.
\end{proof}
