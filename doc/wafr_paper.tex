...
\begin{proof}
Since by Proposition \ref{prop:twosafe} every segment in the partitioned polgyon
$P'$ has at least two safe contracting transitions leaving the segment, and
there are a finite number of segments in $P'$, there must be a strategy reaching
a limit cycle from every point in $P$. \qed
{\color{red} TODO clarify that the fixed-angle controller will be in range of angles
close to boundary}
\end{proof}
...
\begin{proof}
By Corollaries \ref{coro:neighbor} and \ref{coro:existcontract}, if a segment
$e_i$ has an adjacent segment not separated by a reflex angle, a safe
contracting transition exists between the segments.
