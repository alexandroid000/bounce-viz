...
%%S Maybe too redundant after abstract?
...
strategies.
%%S Above, how can an angle be fixed and monotonic?  Fixed means contant, so sure it is monotonic!
...
Although visibility is a symmetric property, we use directed edges so that 
...
There exists a range of angles which result in contracting transitions for transitions between two adjacent 
segments $e_i$ and $e_{i+1}$ where the vertex angle between them is less than $\pi$ (a non-reflex
angle).
\end{corollary}
